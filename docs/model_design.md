...
- **Holdout**: a *separate* full pipeline run with a different
  `population_seed` (e.g. `777`) and different outcome-noise seeds --  a
  distinct simulated cohort from the same target set and the same
  generative process, not a subsample of the training cohort.
